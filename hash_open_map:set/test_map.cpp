...
typedef ics::HashOpenMap<std::string,int,hash_string> MapTypeStr;
typedef ics::HashOpenMap<int,int,hash_int>            MapTypeInt;
typedef ics::HashOpenMap<std::string,int>             MapTypeNone;
...
TEST_F(MapTest, constructors) {
  ...
  MapTypeStr m3({EntryType("f",6), EntryType("c",3), EntryType("i",9),
                ...
                EntryType("h",8)});
  ASSERT_TRUE(mapsto(m3,"fcijbdegah",new int[10]{6,3,9,10,2,4,5,7,1,8}));
  ASSERT_EQ(m,m3);
  m.clear();
  ASSERT_NE(m,m3);
  ...
  for (int test=1; test<=5; ++test) {
    ...
    while (erased != test_size) {
      ...
      for (int i=0; i <to_erase; ++i) {
        ASSERT_EQ(erased,(a = lm.erase(values[erased])));
        std::cout << "erased: " << values[erased] << "\t";
        std::cout << "return erase:" << a << "\t";
        std::cout << lm.has_key(values[erased]) << "\t";
        std::cout << lm << std::endl;
